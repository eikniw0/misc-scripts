...
process_input() {
    ...
    _info "compressing '$cyn$base.tar$wht' with '${red}zstd -$COMP_LVL'"
    zstd -$COMP_LVL \
        --threads=0 \
        --quiet \
        --force \
        --rm \
        "$base.tar" || _fatal "zstd failed\n"
    completed=1
    _success

	finalsize="$(ls -lh $base.tar.zst | awk '{print $5}')"
  _msg " $dim[$bold$ylw☆$rst$dim]$rst final size: $mag$finalsize$wht ($cyn$base.tar.zst$wht)\n"
